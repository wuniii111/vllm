...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    # 承接调度器 (Scheduler) 输出，完成批次准备 → 分布式对齐 → 注意力 / KV 缓存准备 → 
    # 模型前向执行 → 流水线并行 (PP) 数据转发全流程，是调度层和模型计算层的桥梁
    @torch.inference_mode()
    def execute_model(
            self,
        ...
        # 非空跑场景：更新请求状态、释放/新增/刷新请求、落地KV缓存表的临时修改
        if not dummy_run:
            # 更新请求运行状态（结束请求标记）
            self.finish_requests(scheduler_output)
            # 释放已完成请求占用的KV缓存、显存等资源
            self.free_states(scheduler_output)
            # 将调度器新选出的请求加入运行队列
            self.add_requests(scheduler_output)
            # 更新存量请求的上下文、token计数等状态
            self.update_requests(scheduler_output)
            # 批量应用块表(Block Table)的延迟写入，更新KV缓存映射
            self.block_tables.apply_staged_writes()
            # 本轮调度无任何token需要推理，直接返回空结果，跳过模型前向
            if scheduler_output.total_num_scheduled_tokens == 0:
                # 调用KV连接器生成空前向输出
                empty_output = self.kv_connector.no_forward(scheduler_output)
                return empty_output

        # 获取本轮调度的请求数、总token数、单请求最大长度
        num_reqs = len(scheduler_output.num_scheduled_tokens)
        num_toks = scheduler_output.total_num_scheduled_tokens
        max_query_len = max(scheduler_output.num_scheduled_tokens.values())
        # 计算均匀化token数，用于CUDA Graph、DP分布式批次对齐
        uniform_tok_count = get_uniform_token_count(num_reqs, num_toks, max_query_len)

        # 标记是否跳过编译/使用Eager模式
        skip_compiled = False
        # 编解码架构模型（如Whisper）且有编码器输入时，强制走Eager模式
        if self.is_encoder_decoder and scheduler_output.scheduled_encoder_inputs:
            # 编解码模型动态更新交叉注意力KV缓存，无法使用编译/CUDA Graph
            skip_compiled = True

        # 分发批次描述、多DP rank间同步token数量，确定CUDA Graph运行模式
        batch_desc, num_tokens_across_dp = dispatch_cg_and_sync_dp(
            ...
            self.dp_size,
            self.dp_rank,
            # 性能 profiling 或需要Eager时，禁用CUDA Graph
            need_eager=is_profile or skip_compiled,
        )

        # 当前DP rank无待推理token，直接返回空输出
        if batch_desc.num_tokens == 0:
            empty_output = self.kv_connector.no_forward(scheduler_output)
            return empty_output

        # 正常推理流程（非空跑）：准备输入数据与注意力相关元数据
        if not dummy_run:
            # 组装本轮批次的模型输入，拷贝数据到预分配输入缓冲区
            input_batch = self.prepare_inputs(scheduler_output, batch_desc)
            # 准备注意力所需块表、slot映射（KV缓存物理位置映射）
            ...
            if not skip_attn_for_dummy_run:
                ...
                # 全量CUDA Graph模式下，空跑也必须构造注意力元数据，否则图执行异常
        ...
        # 非「空跑+跳过注意力」场景，构建分层slot映射与注意力元数据
        if not (dummy_run and skip_attn_for_dummy_run):
            # 校验slot映射非空
            assert slot_mappings is not None
            # 按模型层拆分slot映射，适配分层KV缓存
        ...
        # 多模态输入embedding（图文/音文等）
        inputs_embeds = None
        # 模型支持多模态、且是流水线并行(PP)第一个rank，负责编码多模态输入
        if self.supports_mm_inputs and self.is_first_pp_rank:
            # 提取多模态编码器输出，得到视觉/音频等embedding
            # 空跑也必须执行，保证编译图输入结构合法
        ...
        # 流水线并行(PP)非首rank：不再接收原始input_ids/embeds，改用中间张量接力
        if not self.is_first_pp_rank:
            # 清空原始输入，避免重复计算
        ...
        # 模型前向执行分支：区分CUDA Graph全量回放 / Eager/分段Graph模式
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            # 全量CUDA Graph模式：直接回放预录制的计算图，无需传参（数据已在缓冲区）
            assert self.cudagraph_manager is not None
            # KV缓存前向预处理（更新缓存状态、指针等）
            self.kv_connector.pre_forward(scheduler_output)
            # 执行CUDA Graph回放，得到模型输出
            model_output = self.cudagraph_manager.run_fullgraph(batch_desc)
        else:
            # 分段CUDA Graph / Eager 模式：直接调用model执行前向
            batch_descriptor = BatchDescriptor(
                num_tokens=input_batch.num_tokens_after_padding,
                has_lora=self.lora_config is not None,
            )

            # 上下文管理器：设置前向全局环境（注意力、图模式、批次信息等）
        ...
        # 流水线并行(PP)最后一个rank：解析最终输出（logits/隐藏态）
        if self.is_last_pp_rank:
            # 区分是否输出额外辅助隐藏态
            if self.use_aux_hidden_state_outputs:
            ...
            # 非最后PP rank：输出为中间特征张量，传递给下一级rank
            assert isinstance(model_output, IntermediateTensors)
            hidden_states = None
            aux_hidden_states = None
            output_intermediate_tensors = model_output

        # KV缓存后处理：更新KV缓存、落地增量key/value
        kv_connector_output = self.kv_connector.post_forward(scheduler_output)
        # 保存本轮执行全量状态，供后续流程/监控使用
