...
class InputBatch:
    ...
    def update_req_spec_token_ids(
        ...
        # 获取当前请求唯一ID
        req_id = request.req_id
        # 通过req_id映射拿到该请求在批量数组里的下标
        req_index = self.req_id_to_index[req_id]
        # 取出当前请求存储草稿token的列表（self.spec_token_ids是二维数组：[batch_idx][草稿token列表]）
        ...
        # 取出当前请求本轮调度好的草稿token，无则返回空元组
        spec_token_ids = scheduled_spec_tokens.get(req_id, ())
        # print(f"Scheduled spec tokens for request {req_id}: {spec_token_ids}")
