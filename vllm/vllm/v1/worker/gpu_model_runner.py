...
class GPUModelRunner(
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        # print(scheduler_output.scheduled_spec_decode_tokens)
    ...
    # 属于 vLLM V1 Scheduler 调度侧代码，作用：
    # 根据每轮调度的草稿 token 数量，算出 target/bonus 两类 logits 在全局 target 模型输出 logits 大张量里的索引，
    # 给后面 RejectionSampler 切分 logits、取草稿 token 做校验用。
    ...
    def execute_model(
        self,
        scheduler_output: "SchedulerOutput",  # 调度器输出：包含本次批处理所有请求、token、KV缓存规划等信息
        intermediate_tensors: IntermediateTensors | None = None,  # PP流水线中间张量，非最后一层rank传递用
    ) -> ModelRunnerOutput | AsyncModelRunnerOutput | IntermediateTensors | None:
        # 状态校验：上一轮execute_model未清空状态时禁止再次调用，必须先执行sample_tokens
        ...
        # 路由专家捕获器初始化清理（MoE专家负载采集）
        if self.routed_experts_initialized:
            capturer = RoutedExpertsCapturer.get_instance()
            if capturer is not None:
                capturer.clear_buffer()  # 清空上一批专家路由统计缓存
            else:
                logger.error("RoutedExpertsCapturer not initialized.")

        # 针对GPU ngram投机解码：拷贝调度输出对象，防止引擎主进程原始数据被原地修改污染
        if (
            self.speculative_config is not None
            and self.speculative_config.use_ngram_gpu()
        ):
        ...
        # 全局统计：本批次总调度token数
        num_scheduled_tokens = scheduler_output.total_num_scheduled_tokens
        # 性能埋点上下文 + 输入数据GPU同步上下文
        with (
            record_function_or_nullcontext("gpu_model_runner: preprocess"),
            self.synchronize_input_prep(),
        ):
            # 更新持久化批处理状态，返回延迟状态修正函数（后置执行）
            deferred_state_corrections_fn = self._update_states(scheduler_output)

            # 编码器缓存迁移场景且当前节点非消费端：仅执行编码器前向，提前返回空输出
            ...
            if not num_scheduled_tokens:
                # 外部启动器DP多数据并行特殊兜底：空批次也执行dummy run保证DP各卡批次对齐
                if (
            ...
            # 快速预填充KV共享开关校验：开启时不支持输出prompt logprob
            if self.cache_config.kv_sharing_fast_prefill:
            ...
            # 转为int32 numpy数组用于后续计算
            num_scheduled_tokens_np = np.array(tokens, dtype=np.int32)
            # 单请求最大调度token长度（用于padding）
            max_num_scheduled_tokens = int(num_scheduled_tokens_np.max())
            # 未padding原始总token数
            num_tokens_unpadded = scheduler_output.total_num_scheduled_tokens

            # 预处理：生成logits采样索引、投机解码元数据
            ...
            # 判定CUDA Graph模式、批次描述、是否启用微批次、DP总token、Graph统计信息
            (
                cudagraph_mode,
                batch_desc,
                should_ubatch,
            ...
            # padding后总token数（适配CUDA Graph固定shape）
            num_tokens_padded = batch_desc.num_tokens
            # padding后请求数，无padding则用原始请求数
            num_reqs_padded = (
                batch_desc.num_reqs if batch_desc.num_reqs is not None else num_reqs
            )
            # 生成微批次分片：原始无padding分片 / padding后分片
            ...
            # 判断是否存在独立KV更新逻辑（注意力内核不内置KV写入，需单独执行）
            ...
            # 全量CUDA Graph模式必须padding注意力输入
            pad_attn = cudagraph_mode == CUDAGraphMode.FULL

            # Mamba缓存对齐模式预处理逻辑
            if self.cache_config.mamba_cache_mode == "align":
                # Mamba预处理依赖最新状态，先执行延迟状态修正
                ...
                # 同步已接受token计数到GPU，保证Mamba内核读取正确初始slot
                self.num_accepted_tokens.np[:num_reqs] = (
                    self.input_batch.num_accepted_tokens_cpu[:num_reqs]
                )
            ...
            # 标记本批次是否启用投机解码
            use_spec_decode = len(scheduler_output.scheduled_spec_decode_tokens) > 0
            # 注意力计算使用padding后分片（需要padding时）
            ubatch_slices_attn = ubatch_slices_padded if pad_attn else ubatch_slices

            # 生成KV缓存slot映射（按分组+全局），适配padding/独立KV更新场景
            ...
            # 输入预处理：input_ids/embedding/位置编码/中间张量/模型扩展参数/编码器迁移输出
            (
                input_ids,
                inputs_embeds,
                positions,
        ...
        # 计算KV缩放值时禁用CUDA Graph（动态计算不支持捕获）
        if self.calculate_kv_scales:
            cudagraph_mode = CUDAGraphMode.NONE
            self.calculate_kv_scales = False  # 仅首遍前向计算一次KV scale

        # 编码器-解码器模型存在编码器输入时强制Eager，不编译CUDA Graph
        num_encoder_reqs = len(scheduler_output.scheduled_encoder_inputs)
        has_encoder_input = (
            self.model_config.is_encoder_decoder and num_encoder_reqs > 0
        )

        # 投机解码场景延迟KV连接器收尾，等Draft模型执行完成再释放元数据
        defer_kv_connector_finalize = self.speculative_config is not None
        # 前向计算上下文：全局前向上下文埋点、性能profile、KV迁移连接器上下文
        with (
            set_forward_context(
                attn_metadata,
                self.vllm_config,
        ...
        # 缓存本轮完整运行状态，供后续sample_tokens采样使用
        ...
        # 前向执行完成后，再执行延迟状态修正函数，不阻塞GPU计算流水线
        if deferred_state_corrections_fn:
            deferred_state_corrections_fn()

        # 返回None标识execute_model执行完成，外层调用sample_tokens读取execute_model_state
        return None
    ...
    def propose_draft_token_ids(  # NPURunner重写
        self,
        scheduler_output: "SchedulerOutput",
