...
class SpecDecodingProm:
    ...
    def observe(self, spec_decoding_stats: SpecDecodingStats, engine_idx: int = 0):
        ...
        # 增加边界保护，防止下标越界  ###改
        counter_list = self.counter_spec_decode_num_accepted_tokens_per_pos[engine_idx]
        stat_list = spec_decoding_stats.num_accepted_tokens_per_pos
        max_pos = min(len(counter_list), len(stat_list))
        for pos in range(max_pos):
            counter_list[pos].inc(stat_list[pos])
