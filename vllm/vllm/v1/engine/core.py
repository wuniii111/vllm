...
class EngineCore:
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        with (
            ...
            if model_output is None:
                model_output = self.model_executor.sample_tokens(grammar_output)  # 在这儿
    ...
    def post_step(self, model_executed: bool) -> None:
        ...
        # 异步调度时草稿 token 的回收在 Worker 进程内完成
        if not self.async_scheduling and self.use_spec_decode and model_executed:
            # Take the draft token ids.
            # 拿到draft token ids后，scheduler会在下一轮step()中使用这些token ids来计算grammar bitmask
            draft_token_ids = self.model_executor.take_draft_token_ids()
            print("post_stee", draft_token_ids)
    ...
    │
    ├── 1️⃣ 如果 scheduler 还有请求
    │   ├─ schedule() → scheduler_output
    │   ├─ execute_model(non_block=True)
    │   ├─ sample_tokens() or defer
    │   └─ 把 (future, scheduler_output) 塞进 batch_queue
    │
    ├── 2️⃣ 如果 queue 还没满 → 立刻返回（不阻塞）
    │
    ├── 3️⃣ 否则（queue 满 / 没新请求）
    │   └─ 阻塞等最早一个 batch 完成
    │
    ├── 4️⃣ 拿 model_output
    │   ├─ update scheduler
    │   └─ 处理 deferred speculative decoding
    │
    ...
    def step_with_batch_queue(
        ...
        if self.scheduler.has_requests():
            scheduler_output = self.scheduler.schedule()  # scheduler_output.num_scheduled_tokens {'chatcmpl-9afb...': 14} scheduler_output.scheduled_spec_decode_tokens
            ...
            if self.is_pooling_model or not model_executed:
                ...
                if not scheduler_output.pending_structured_output_tokens:
                    ...
                    # 为当前这一批请求生成 结构化输出（JSON / Regex / Grammar）约束掩码（bitmask）
                    grammar_output = self.scheduler.get_grammar_bitmask(
                        scheduler_output
                    )
                    future = self.model_executor.sample_tokens(  # 返回前8个token
        ...
        # Future是一个异步计算结果的占位对象（Promise），代表"某个操作将来会产生结果，但现在还没好"
        ...
        if deferred_scheduler_output:
            ...
            if self.use_spec_decode:
                ...
                # 把无效、被拒绝的 draft token 统一填充为 -1
                # 后面语法掩码计算时会直接跳过值为 -1 的 token，避免非法字符破坏 JSON 语法约束
...
class EngineCoreProc(EngineCore):
    ...
    def run_busy_loop(self):
        ...
        while self._handle_shutdown():
            # 1) Poll the input queue until there is work to do. 消费外部请求
    ...
    def _process_engine_step(self) -> bool:
        ...
        # 推测解码（Speculative Decoding）场景：从上一步 ModelRunner 的 drafter（你上一问看的 
        # propose_draft_token_ids()结果）中取出本轮生成的 draft_token_ids，回写给 Scheduler，
        # 让下一轮调度时把这些 draft token 塞进 batch 去做 verify（accept/reject）
        # 非推测解码或 async_scheduling 模式时，post_step()基本是空操作
