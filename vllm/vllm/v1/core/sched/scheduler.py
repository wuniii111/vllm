...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            num_new_tokens = (
                ...
                request.num_tokens_with_spec  # 33 len(self._all_token_ids)18 + len(self.spec_token_ids)15
                + request.num_output_placeholders  # 占位符 token 数 0  已分配但采样结果尚未返回 16
                - request.num_computed_tokens  # 已经计算出来的 token 数 1024(prompt) 或 P+5（上次只算了到已接受位置） 33
            )
            if 0 < self.scheduler_config.long_prefill_token_threshold < num_new_tokens:
                num_new_tokens = self.scheduler_config.long_prefill_token_threshold
            num_new_tokens = min(num_new_tokens, token_budget)  # 截断到剩余的 token 预算
            ...
            if request.spec_token_ids:
                # 1. 计算本轮需要校验的spec token数量 15
                num_scheduled_spec_tokens = (
                    num_new_tokens  # 本轮实际分配的 token 数 16 16
                    + request.num_computed_tokens  # 14 30
                    - request.num_tokens  # 14 15
                    - request.num_output_placeholders  # 1 16 提前生成 N 个 draft token 塞进序列占位
                )
                # 2. 数量合法才截取存入
                if num_scheduled_spec_tokens > 0:
                    spec_token_ids = request.spec_token_ids
                    # 截断到本轮可调度的spec token数量
                    if len(spec_token_ids) > num_scheduled_spec_tokens:
                        spec_token_ids = spec_token_ids[:num_scheduled_spec_tokens]
                    # 写入全局调度spec字典
                    scheduled_spec_decode_tokens[request.request_id] = spec_token_ids  ### 更新scheduled_spec_decode_tokens
                # 3. 清空当前请求缓存的spec_token_ids
                request.spec_token_ids = []
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                # num_scheduled_tokens是当前批次中每个请求调度的token数
                num_scheduled_tokens[request_id] = num_new_tokens
                token_budget -= num_new_tokens
    ...
    # 把一个正在 RUNNING 的请求强行踢出当前 batch，清空它的 KV Cache，让它重新排队等待下次调度
