...
# 昇腾专用注意力元数据构造器，继承vLLM通用注意力元数据构建基类，输出昇腾NPU专属AscendMetadata
class AscendAttentionMetadataBuilder(AttentionMetadataBuilder[AscendMetadata]):
    ...
    # 当前后端是否会对batch内请求重排序；None代表不重排，数值代表会把查询长度小于该值的请求挪到batch头部
    reorder_batch_threshold: int = 1

    def __init__(
        self,
        kv_cache_spec: AttentionSpec,          # KV缓存规格描述，包含块大小、并行、压缩等KV配置
        layer_names: list[str],                # 模型各注意力层名称列表
        vllm_config: VllmConfig,                # vLLM全局总配置对象，包含模型、调度、推测、编译等所有参数
        ...
        # 提取模型基础配置（上下文长度、头数、模型尺寸等）
        self.model_config = vllm_config.model_config
        # 提取编译/计算图捕获相关配置
        self.compilation_config = vllm_config.compilation_config
        # 保存NPU设备对象
        self.device = device
        # 计算单个请求能占用的最大KV块数量：最大上下文长度 / 昇腾注意力内核支持的块尺寸
        self.max_num_blocks_per_req = cdiv(
            self.model_config.max_model_len, AscendAttentionBackend.get_supported_kernel_block_sizes()[0]
        )

        # 提取推测解码全局配置（包含num_speculative_tokens）
        self.speculative_config = vllm_config.speculative_config
        # 解码区分阈值，基础值为1：原生单token解码场景
        self.decode_threshold = 1
        # 如果开启了推测解码，阈值叠加单次预生成候选token数量
        if self.speculative_config:
            spec_token_num = self.speculative_config.num_speculative_tokens
            self.decode_threshold += spec_token_num
            # 昇腾NPU融合注意力算子TND布局硬性限制：单次解码总token不能超过16，超出会算子报错
        ...
        # 将batch重排序阈值和推测解码后的解码阈值绑定
        # 区分 Prefill 和 Decode 请求的阈值；开启推测解码后，单次解码循环最多同时处理 1+k 个 token
        # reorder_batch_threshold用于控制调度器是否把「短查询解码请求」挪到批次最前面优先计算
        self.reorder_batch_threshold = self.decode_threshold
        
        # 提取调度器配置
        scheduler_config = vllm_config.scheduler_config
        # 是否开启分块预填充（超长prompt分段并行prefill）
        self.chunked_prefill_enabled = scheduler_config.enable_chunked_prefill
        # 初始化注意力掩码构造工具，用于生成因果掩码/填充掩码
    ...
    def get_cudagraph_support(
        ...
        # 重写父类方法，声明昇腾后端全程支持NPU计算图捕获（对标CUDA Graph）
        # @override 注解省略是mypy类型变量限制导致
        return AttentionCGSupport.ALWAYS

    def reorder_batch(self, input_batch, scheduler_output: "SchedulerOutput") -> bool:
        # 当前昇腾后端不做batch请求重排序，直接返回False
        return False

    def build(
        self,
        common_prefix_len: int,                        # 批量请求共享的公共前缀token长度（预填充优化用）
        common_attn_metadata: AscendCommonAttentionMetadata, # 通用批注意力中间元数据，存放所有批原始信息
        fast_build: bool = False,                      # 是否启用快速构建模式（跳过部分冗余校验）
    ) -> AscendMetadata:
        # 当前批内有效请求总数
        num_reqs = common_attn_metadata.num_reqs
        # 当前批全部token总数量（prefill+decode+spec候选token）
        num_actual_tokens = common_attn_metadata.num_actual_tokens
        # CPU侧请求token起始偏移数组，长度num_reqs+1，用于划分每个请求的token区间
        query_start_loc_cpu = common_attn_metadata.query_start_loc_cpu[: num_reqs + 1]

        # 根据decode_threshold拆分当前批为解码请求、预填充请求，同时统计两类token总数
        num_decodes, num_prefills, num_decode_tokens, num_prefill_tokens = split_decodes_and_prefills(
            common_attn_metadata, decode_threshold=self.decode_threshold
        )

        # 获取块表张量：存储每条请求对应的KV物理块编号
        block_table = common_attn_metadata.block_table_tensor
        # 优先读取_seq_lens_cpu：推测解码迭代过程实时更新的序列长度；普通seq_lens在异步spec场景会为空
        ...
        # 截取有效长度的slot_mapping，每个元素代表单个token对应的KV显存槽位ID
        slot_mapping = common_attn_metadata.slot_mapping[:num_actual_tokens]
        # 注释：原生vLLM会二次覆盖slot_mapping，此处临时修改不生效，待上游vLLm修复
        # 跨注意力分支（多模态图像编码器等场景）特殊处理
        if isinstance(self.kv_cache_spec, CrossAttentionSpec):
            seq_lens = common_attn_metadata.seq_lens
            slot_mapping = common_attn_metadata.slot_mapping.to(torch.int32)
        # 并行Draft推测解码分支，使用设备侧原始序列长度
        elif self.speculative_config and self.speculative_config.parallel_drafting:
            seq_lens = common_attn_metadata.seq_lens

        # 获取批推理状态标识（仅解码/分块预填充/推测解码）
        attn_state = common_attn_metadata.attn_state

        # 调用掩码工具生成因果注意力掩码，传入是否因果、模型配置
        attn_mask = self.attn_mask_builder.get_attention_mask(common_attn_metadata.causal, self.model_config)

        # 将CPU侧请求偏移数组锁页内存，异步拷贝至NPU显存，供给注意力算子使用
        # TODO：存在冗余H2D拷贝，原始元数据已有设备侧query_start_loc可复用
        query_start_loc = query_start_loc_cpu.pin_memory().to(self.device, non_blocking=True)

        # 组装昇腾专属注意力元数据对象，所有参数供给下层FlashAttention算子
        attn_metadata = AscendMetadata(
            num_actual_tokens=num_actual_tokens,            # 批内总token数量
            num_decode_tokens=num_decode_tokens,            # 解码阶段token总数（含spec候选token）
            block_tables=block_table,                       # KV块表张量
            query_start_loc=query_start_loc,                # NPU设备侧请求token起始偏移
            seq_lens=seq_lens,                              # CPU侧每条请求当前上下文长度数组
            seq_lens_cpu=seq_lens,                          # 重复保存CPU序列长度字段，兼容算子入参
            seq_lens_list=seq_lens.tolist(),                # 转为python列表，供上层调度打印/统计
            max_query_len=common_attn_metadata.max_query_len,# 当前批单条请求最大token长度
            actual_seq_lengths_q=query_start_loc_cpu[1:].tolist(), # 每条请求自身query的token长度列表
            slot_mapping=slot_mapping,                      # KV槽位映射数组，寻址KV缓存核心
            attn_mask=attn_mask,                            # 注意力掩码（因果/填充屏蔽）
            attn_state=attn_state,                          # 当前推理状态：解码/预填充/推测解码
            num_prefills=num_prefills,                      # 批内预填充请求数量
            num_decodes=num_decodes,                        # 批内解码请求数量
            causal=common_attn_metadata.causal,             # 是否开启因果注意力（LLM必开True）
            model_runner_type=self.model_config.runner_type,# 模型执行器类型（310P专属runner标识）
            kvcomp_metadata=common_attn_metadata.kvcomp_metadata, # KV缓存压缩元数据（开启KV压缩时生效）
        )
        # 返回组装完成的昇腾注意力元数据，送入注意力前向计算
        return attn_metadata

    # 计算图捕获专用构建函数：生成固定shape的dummy元数据，用于提前编译NPU计算图
    def build_for_graph_capture(
        self,
        common_attn_metadata: AscendCommonAttentionMetadata,
        attn_state: AscendAttentionState = AscendAttentionState.DecodeOnly,
    ):
        # 仅支持纯解码、分块预填充、推测解码三种状态的图捕获
        if attn_state in (
            AscendAttentionState.DecodeOnly,
            AscendAttentionState.ChunkedPrefill,
            AscendAttentionState.SpecDecoding,
        ):
            # 调用通用build构造空元数据，公共前缀长度填0
            attn_metadata = self.build(
                common_prefix_len=0,
                common_attn_metadata=common_attn_metadata,
            )
        else:
            # 其他推理状态暂不支持图捕获，抛出未实现异常
