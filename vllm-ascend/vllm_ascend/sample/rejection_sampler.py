...
class AscendRejectionSampler(RejectionSampler):
    ...
    def forward(
            ...
            metadata：推测解码专用元数据
            draft_probs (可选张量)
            草稿模型输出的 token 概率分布，张量形状为 [总草稿token数, 词表大小]。
            ngram 推测解码场景下该参数为空（None），不会提供概率值。
            logits
            大目标模型输出的原始对数概率分布，张量形状为 [总草稿token数 + 批次请求数, 词表大小]。
            框架会把所有请求的计算结果展平合并为单个张量，这是模型输出 logits 的标准格式。
            注：该张量支持原地修改，可用于节省显存。
            sampling_metadata（采样元数据）
            采样流程所需的各类超参与附加信息，包含温度系数、top-k、top-p 等采样控制参数
            返回值:
            SamplerOutput 采样输出结构体
            存储经过校验、最终确定输出的 token ID；若用户开启对数概率计算，会同步携带各 token 的对数概率信息。
            """
            """
            Args:
                metadata:
            ...
            Returns:
                SamplerOutput:
                    ...
                    requested.
            """
            # 校验推测解码最大生成长度不超过全局限制常量
            assert metadata.max_spec_len <= MAX_SPEC_LEN
            # 从推测解码元数据取出bonus token对应的logits索引
            bonus_logits_indices = metadata.bonus_logits_indices
            # 从推测解码元数据取出所有draft候选token对应的目标模型logits索引
            target_logits_indices = metadata.target_logits_indices

            # 当使用张量索引切片logits时，PyTorch会创建存储独立的新张量
            # 因此对bonus_logits原地修改不会影响原始总logits
            assert logits is not None
            # 根据索引切片，提取所有bonus token对应的原始logits
            bonus_logits = logits[bonus_logits_indices]
            # 调用基础采样器对bonus logits做采样，生成追加候选token
            bonus_sampler_output = self.sampler(
                logits=bonus_logits,
                sampling_metadata=replace(
                    sampling_metadata,
                    max_num_logprobs=-1,  # 临时关闭logprobs计算，仅用于采样bonus token
                ),
                predict_bonus_token=True,  # 标记本次采样是生成bonus补偿token
                # 根据模式覆盖logprob计算方式：预处理后logits / 原始未处理logits
                logprobs_mode_override="processed_logits" if self.is_processed_logprobs_mode else "raw_logits",
            )
            # 取出采样得到的bonus补偿token id
            bonus_token_ids = bonus_sampler_output.sampled_token_ids

            # 同bonus_logits，target_logits是独立存储切片，原地修改安全，不污染原始logits
            raw_target_logits = logits[target_logits_indices]
            # 将目标模型原始logits转换为float32提升数值计算稳定性
            raw_target_logits = raw_target_logits.to(torch.float32)
            # 初始化待处理的目标logits变量
            target_logits = raw_target_logits
            if not self.is_processed_logprobs_mode:
                # 若使用原始logits计算logprob，提前克隆一份原始logits备份
                # 因为后续logits处理器会原地修改张量，避免原始值被覆盖无法算logprob
                target_logits = target_logits.clone()

            ### 对目标模型logits执行各类约束/加权处理（温度、topP、惩罚等）
            target_logits = self.apply_logits_processors(target_logits, sampling_metadata, metadata)
            # [num_tokens, vocab_size]
            # 执行采样硬约束（如禁止词、长度惩罚、topk截断），支持原地更新logits
            target_logits = apply_sampling_constraints(
                target_logits, metadata.cu_num_draft_tokens, sampling_metadata, self.top_k
            )

            # 执行推测解码核心拒绝采样逻辑：对比draft模型分布与目标模型分布，校验候选token
            output_token_ids = rejection_sample(
                metadata.draft_token_ids,          # 草稿模型生成的候选token序列
                metadata.num_draft_tokens,          # 每个请求的草稿token数量
                metadata.max_spec_len,              # 单次推测最大token长度上限
                metadata.cu_num_draft_tokens,       # 各请求草稿token累计偏移（分段索引）
                draft_probs,                        # 草稿模型输出概率分布
                target_logits,                      # 处理后的目标模型logits
                bonus_token_ids,                    # 预先生成的bonus补偿token
                sampling_metadata,                  # 采样超参元数据
            )

            # 初始化logprob输出张量为空
            logprobs_tensors = None
            # 如果用户配置需要返回token对数概率，则计算完整logprob数据
            if sampling_metadata.max_num_logprobs is not None:
                logprobs_tensors = self._get_logprobs_tensors(
                    sampling_metadata.max_num_logprobs,    # 需要返回的top-k logprob数量
                    metadata,                               # 推测解码全局元数据
                    logits,                                 # 原始完整未切片总logits
                    # 根据模式选择预处理后/原始logits用于计算logprob
                    target_logits if self.is_processed_logprobs_mode else raw_target_logits,
                    bonus_sampler_output.logprobs_tensors.logprobs, # bonus token对应的logprob
                    output_token_ids,                       # 最终校验通过的输出token id
                )

            # 封装采样结果返回：最终token序列 + 可选logprob信息
