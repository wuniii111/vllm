...
class NPUModelRunner(GPUModelRunner):
    ...
    def _prepare_inputs(
            self,
            scheduler_output: "SchedulerOutput",  # 调度器输出：批次所有请求、草稿token、调度token数等全局信息
            num_scheduled_tokens: np.ndarray,     # np数组，每个请求本轮调度的token数量
        ) -> tuple[torch.Tensor, SpecDecodeMetadata | None, int, list[np.ndarray[Any, Any]]]:
            """
            :return: tuple[
                logits_indices,                     # 所有需要计算logit的hidden_states索引（基准token+全部草稿token）
                spec_decode_metadata,               # 投机解码全套元数据（草稿id、草稿索引、校验索引等，无投机则None）
                total_num_scheduled_tokens,         # 本轮批次总token数量（PCP拆分后真实总数）
                num_scheduled_tokens_compressed_list# DSA压缩KV专用各层token分片数组
            ]
            """
            # 读取调度器原始总token数（未经过PC拆分）
            total_num_scheduled_tokens = scheduler_output.total_num_scheduled_tokens
            # 批次token必须大于0，空批次上层提前拦截，此处断言防御
            ...
            # 优化：提前提交block_table拷贝到NPU，与后续CPU计算流水重叠，隐藏数据传输耗时
            self.input_batch.block_table.commit_block_table(num_reqs)

            # req_indices：每个token归属请求下标，形状[total_tokens]
            # 示例：req0有3个token → [0,0,0], req1有2个→[1,1]
            req_indices = np.repeat(self.arange_np[:num_reqs], num_scheduled_tokens)

            # ====================== 区分预填充/解码状态 ======================
            # 判断是否存在投机解码草稿token
            if not scheduler_output.scheduled_spec_decode_tokens:
                # 无草稿：有效token = 本轮调度全部token
                num_valid_tokens = num_scheduled_tokens
            else:
                # 有草稿：有效token = 总调度token - 该请求草稿token数量
            ...
            # ====================== PCP上下文并行前置slot计算 ======================
            # PCP开启时，先在拆分前计算原始位置的slot映射，避免分片后位置错乱
            if self.pcp_size > 1:
                # CPU位置转NPU张量，阻塞拷贝保证数据就绪
                pre_pcp_positions = torch.from_numpy(
                    positions_np[:total_num_scheduled_tokens]
                ).to(self.device)
                # 初始化query_start_loc前置张量，存储各请求token边界
            ...
            # PCP+投机解码：生成MTP草稿输入分片信息，适配拆分后的序列
            if self.speculative_config and self.use_cp:
                self.pcp_manager.generate_pcp_mtp_input(
                    ...
                    positions_np,
                    cu_num_tokens,
                    self._draft_token_ids,  # 上一轮生成的草稿token
                    scheduler_output,
            ...
            # PCP混合注意力模式，加载padding后的单请求token长度
            ...
            # ====================== input_ids 索引构建 ======================
            # token_indices：计算token在全局大prompt数组中的扁平下标
            # 公式：全局位置 + 请求ID * 单请求最大长度，区分不同请求的prompt存储区域
            token_indices = positions_np + req_indices * self.input_batch.token_ids_cpu.shape[1]
            token_indices_tensor = torch.from_numpy(token_indices)
            # 从CPU缓存批量索引取出本轮input_ids写入预分配CPU缓冲区
            torch.index_select(
            ...
            # ====================== 多模态Prompt Embedding填充 ======================
            # 存在图文输入时，把预计算图片embedding填入inputs_embeds缓冲区
            if self.input_batch.req_prompt_embeds and (self.is_multimodal or self.enable_prompt_embeds):
                output_idx = 0
                for req_idx in range(num_reqs):
                    ...
                    # PCP拆分场景：token不连续，按拆分后的真实位置填充embedding
                    if self.pcp_size > 1:
            ...
            # GDN注意力需要无padding原始query_start_loc，单独维护一份
            if self._has_gdn:
            ...
            # ====================== 乐观序列长度（假设所有草稿全部接受） ======================
            # 乐观seq_len = 已计算token + 本轮调度token，用于注意力max_seq_len计算
            torch.add(
            ...
            # 异步调度：建立上一轮请求ID→当前下标映射，用于状态恢复
            prev_req_id_to_index = self.input_batch.prev_req_id_to_index
            self._compute_prev_positions(num_reqs)

            # 未使用请求边界填充-1，适配注意力cache重排内核
            self.query_start_loc.gpu[num_reqs + 1 :].fill_(-1)

            # input_ids、positions等CPU缓冲区批量拷贝到NPU设备
            self._prepare_input_ids(scheduler_output, num_reqs, total_num_scheduled_tokens, cu_num_tokens)

            # M-RoPE多模态位置编码计算（Qwen-VL等图文模型）
            if self.uses_mrope:
            ...
            # ====================== 同步已接受草稿token数量 ======================
            # 上一轮采样完成事件就绪，同步CPU已接受token计数
            if self.num_accepted_tokens_event is not None:
                self.num_accepted_tokens_event.synchronize()
                # 异步调度：根据上一轮请求映射修正accept计数
            ...
            # PCP场景直接使用分片修正后的position缓冲区，不重新计算
            if self.pcp_size > 1:
                ...
                # 普通场景GPU实时计算全局position
                self.positions[:total_num_scheduled_tokens] = (
                    self.num_computed_tokens[req_indices_gpu].to(torch.int64)
                    + self.query_pos.gpu[:total_num_scheduled_tokens]
                )
            # seq_len = 已计算token + 本轮调度token
            self.seq_lens[:num_reqs] = (
                self.num_computed_tokens[:num_reqs] + num_scheduled_tokens_gpu
            )
            self.seq_lens[num_reqs:].fill_(0)

            # 异步投机：修正后的GPU序列长度同步回CPU，供注意力后端读取
            if (
            ...
            # 判断当前批次是否携带上一轮草稿token
            use_spec_decode = len(scheduler_output.scheduled_spec_decode_tokens) > 0
            # print("npu_runner._prepare_inputs_调度下发spec tokens:", scheduler_output.scheduled_spec_decode_tokens)
            if not use_spec_decode:
                ...
                if self.use_cp:
                    ...
                    # 普通场景：每个请求最后一个token下标（query_start_loc末尾-1）
                    logits_indices = self.query_start_loc.gpu[1 : num_reqs + 1] - 1
            else:
                ###
                # ====================== 投机解码核心分支 ======================
                # 初始化每个请求草稿token数量数组，默认0
                num_draft_tokens = np.zeros(num_reqs, dtype=np.int32)
                new_schedule_reqs = [x.req_id for x in scheduler_output.scheduled_new_reqs]
                # 解码草稿掩码，预填充-1（prefill阶段标记无效）
                num_decode_draft_tokens = np.full(num_reqs, -1, dtype=np.int32)
                # 遍历调度器草稿字典，填充每个请求草稿长度
                for (
                    req_id,
                    draft_token_ids,
                ...
                # 调用函数生成全套投机元数据：草稿id、target校验索引、logit索引
                ...
                # GDN解码图专用草稿长度缓冲区同步GPU
                self.num_decode_draft_tokens.np[:num_reqs] = num_decode_draft_tokens
                self.num_decode_draft_tokens.np[num_reqs:].fill(-1)
                self.num_decode_draft_tokens.copy_to_gpu()
            # 缓存logits索引，供PCP投机解码后续使用
            self.logits_indices = logits_indices

            # LoRA动态切换：激活当前批次请求对应的微调权重
            if self.lora_config:
            ...
            # 返回四元组：logit索引、投机元数据、总token、压缩分片列表
            return (
                logits_indices,
    ...
    def _calc_spec_decode_metadata(
            self,
            num_draft_tokens: np.ndarray,       # 每个请求生成的草稿token数量，示例 [3,0,2,0,1]
            cu_num_scheduled_tokens: np.ndarray,# 每个请求总调度token累积和，示例 [4,104,107,207,209]
            num_pcp_pads: np.ndarray | None,    # PCP上下文并行时填充token数量，无PCP则为None
        ) -> SpecDecodeMetadata:
            # 注释示例输入：
            # cu_num_scheduled_tokens: 每个请求累计token总和 [  4, 104, 107, 207, 209]
            # num_draft_tokens:        每个请求草稿token个数 [  3,   0,   2,   0,   1]
            # 输出字段说明：
            # cu_num_draft_tokens:      草稿token累积和 [  3,   3,   5,   5,   6]
            # logits_indices:           所有需要计算logit的token在hidden_states中的索引 [0,1,2,3,103,104,105,106,206,207,208]
            # target_logits_indices:    仅草稿token对应的logit索引（用于和draft模型对比校验）[  0,   1,   2,   5,   6,   9]
            # bonus_logits_indices:    每个请求最后一个基准token索引 [  3,   4,   7,   8,  10]

            # Step 1：每个请求采样总数 = 1个基准token + N个草稿token
            num_sampled_tokens = num_draft_tokens + 1
            # 示例输出：[4, 1, 3, 1, 2]

            # Step 2：计算采样token的累积长度，同时生成每个请求内相对偏移数组
            cu_num_sampled_tokens = self._get_cumsum_and_arange(
                num_sampled_tokens, self._arange_scratch, cumsum_dtype=np.int32
            )
            # 输出cu_num_sampled_tokens：[4, 5, 8, 9, 11]
            # _arange_scratch：记录每个请求内部token相对下标 [0,1,2,3,0,0,1,2,0,0,1]

            # Step3：计算全局基准起始位置（每个请求第一个token的全局下标）
            logits_indices = np.repeat(cu_num_scheduled_tokens - num_sampled_tokens, num_sampled_tokens)
            # 中间结果示例：[0,0,0,0,103,104,104,104,206,207,207]

            # Step4：叠加每个请求内部偏移，得到所有token全局索引
            logits_indices += self._arange_scratch[: cu_num_sampled_tokens[-1]]
            # 最终logits_indices：[0,1,2,3,103,104,105,106,206,207,208]
            # 作用：hidden_states[logits_indices] 取出所有需要算logit的特征（基准+全部草稿）

            # PCP上下文并行特殊处理：存在分片填充，重新生成PCP修正后的logit索引
            if self.pcp_size > 1:
                # 修正PCP分片带来的token偏移
            ...
            # 计算bonus_logits_indices：每个请求最后一个基准token的下标（无草稿的基准输出）
            bonus_logits_indices = cu_num_sampled_tokens - 1
            # 示例：[3,4,7,8,10]，对应每个请求末尾基准token，是无投机时正常输出位置

            # ========== 草稿token索引计算，核心用于target模型校验对比 ==========
            # Step1：计算草稿token累积总数
            cu_num_draft_tokens = np.cumsum(num_draft_tokens, dtype=np.int32)
            # 示例输出：[3,3,5,5,6]
            total_num_draft_tokens = cu_num_draft_tokens[-1] # 全局总草稿token数量
            ...
            # Step5：叠加草稿内部偏移，得到仅草稿token的logit索引（target校验专用）
            target_logits_indices += arange
            # 最终target_logits_indices：[0,1,2,5,6,9]
            # 关键用途：只取出所有草稿位置的target logits，和draft模型输出做拒绝采样校验

            # TODO: 后续优化CPU到NPU的数据拷贝开销
            # 各类索引数组从numpy转cuda张量，页锁定内存加速传输
            ...
            # 提取草稿token真实id
            # 1. 根据全部采样索引取出整段token序列
            draft_token_ids = self.input_ids.gpu[logits_indices]
            # 2. target_logits_indices是草稿对应特征下标，+1取对应草稿token本身
            draft_token_ids = draft_token_ids[target_logits_indices + 1]
            # 示例draft_token_indices [1,2,3,105,106,208]，取出所有草稿token id

            # PCP场景替换索引为分片修正后的版本
            if self.pcp_size > 1:
                logits_indices = logits_indices_pcp

            # 封装投机解码元数据返回，给execute_model、rejection_sampler使用
            return SpecDecodeMetadata(
                draft_token_ids=draft_token_ids,               # 草稿token真实id列表，来自draft模型输出
                num_draft_tokens=num_draft_tokens.tolist(),     # 每个请求草稿数量
                cu_num_draft_tokens=cu_num_draft_tokens,        # 草稿token累积偏移
                cu_num_sampled_tokens=cu_num_sampled_tokens,     # 所有采样（基准+草稿）累积偏移
                target_logits_indices=target_logits_indices,    # 【校验核心】草稿位置target logit索引
                bonus_logits_indices=bonus_logits_indices,       # 纯基准token索引（无草稿兜底）
                logits_indices=logits_indices,                  # 全部token（基准+草稿）全局索引
            )
    ...
    def propose_draft_token_ids(
        ...
        elif self.speculative_config.use_eagle() or self.speculative_config.uses_draft_model():  # 走这个分支
            common_attn_metadata = spec_decode_common_attn_metadata
            sampled_token_ids = valid_sampled_token_ids
    ...
    def _copy_draft_token_ids_to_cpu(
        ...
        draft_token_ids: torch.Tensor = self._draft_token_ids  # type: ignore[has-type]  # 拷贝self._draft_token_ids
    ...
    def execute_model(
        self,
        scheduler_output: "SchedulerOutput",  # 调度器输出：当前批次所有请求、token调度信息
        intermediate_tensors: IntermediateTensors | None = None,  # PP流水线并行中间层输出张量
    ) -> ModelRunnerOutput | IntermediateTensors | None:
        # ========== 1.MoE专家捕获缓存清空 ==========
        # 开启专家路由信息收集时，清空上一轮捕获缓冲区
        if self.vllm_config.model_config.enable_return_routed_experts:
            # vllm 0.20.2版本使用单例捕获器
        ...
        if self.ascend_config.profiling_chunk_config.need_timing:
            ...
            if getattr(scheduler_output, "disable_profiling_timing", False):
                ...
                self._sync_device()  # NPU设备同步，确保之前任务跑完再计时
                self._execution_start_time = time.perf_counter()  # 记录推理起始时间戳

        # ========== 3.状态校验：上一轮采样必须完成才能执行新前向 ==========
        # execute_model_state存储模型输出中间态，必须调用sample_tokens清空
        if self.execute_model_state is not None:
            raise RuntimeError("State error: sample_tokens() must be called after execute_model() returns None.")
    
        # ========== 4.ngram投机解码：拷贝调度输出防止原对象被篡改 ==========
        # GPU版ngram草稿模型会修改scheduler_output，复制副本隔离数据
        if (
        ...
        # ========== 6.异步调度+PCP多模态场景：深拷贝调度对象 ==========
        # 异步投机解码、PC上下文并行+多模态场景会修改调度数据，深拷贝隔离原引擎数据
        # TODO：大批量请求时deepcopy性能差，后续优化
        if ((
            self.use_async_scheduling and self.num_spec_tokens and self._draft_token_ids is None  # type: ignore[has-type]
        ) or (
            # PCP+多模态仅prefill阶段需要深拷贝，上游vllm原生不支持该组合，临时兼容方案
        ...
        with record_function_or_nullcontext("prepare input"):
            ...
            with self.synchronize_input_prep():
                # ========== 7.修复异步调度遗留请求状态 ==========
                # 上一轮被丢弃的请求残留草稿长度置0，避免索引KeyError
                if (
                    self.use_async_scheduling
                ...
                # 更新批次持久化状态，返回延迟修正函数（后置执行）
                deferred_state_corrections_fn = self._update_states(scheduler_output)

                # ========== 8.编码器分流：图文编码任务单独处理 ==========
                # EC跨设备编码器传输生产者，仅执行图片编码，不走LLM前向
                if has_ec_transfer() and get_ec_transfer().is_producer:
                    with self.maybe_get_ec_connector_output(
                        ...
                        self._finalize_dump_data()  # 结束调试采集
                        return make_empty_encoder_model_runner_output(scheduler_output)

                # ========== 9.无token空批次处理 ==========
                if not num_scheduled_tokens:
                    # DP多数据并行+外部启动器，空批次也要dummy_run同步各卡batch
                    if (
                ...
                # ========== 10.快速KV缓存校验：禁止需要prompt logprob的场景 ==========
                # 共享KV快速预填充无法计算prompt对数概率，冲突直接断言报错
                if self.cache_config.kv_sharing_fast_prefill:
                    assert not self.num_prompt_logprobs, (
                        "--kv-sharing-fast-prefill produces incorrect "
                        "logprobs for prompt tokens, please disable "
                        "it when the requests need prompt logprobs"
                    )

                num_reqs = self.input_batch.num_reqs  # 当前批次请求总数
                req_ids = self.input_batch.req_ids    # 批次内请求id列表
                # 每个请求本次调度token数
                tokens = [scheduler_output.num_scheduled_tokens[i] for i in req_ids]
                num_scheduled_tokens_np = np.array(tokens, dtype=np.int32)
                max_num_scheduled_tokens = int(num_scheduled_tokens_np.max())  # 单请求最大token长度

                # ========== 11.核心输入预处理：构造位置、索引、注意力元数据前置信息 ==========
                (
                    logits_indices,                # 每个请求最后一个token索引，用于取hidden_states算logits
                    spec_decode_metadata,          # 投机解码草稿token元数据
                    total_num_scheduled_tokens,    # PCP拆分后真实总token数
                    num_scheduled_tokens_compressed_list,  # 压缩KV专用token分片
                ) = self._prepare_inputs(
                    scheduler_output,
                    num_scheduled_tokens_np,
                )

                num_tokens_unpadded = scheduler_output.total_num_scheduled_tokens  # 原始无padding总token
                # PCP开启时替换为并行拆分后的token总数
                if self.pcp_size > 1:
                    num_tokens_unpadded = self.pcp_manager.total_num_sampled_tokens_pcp

                cascade_attn_prefix_lens = None  # 级联注意力前缀长度（默认关闭）
                # 微批DBO关闭时才启用级联注意力
                ...
                # ========== 12.判断是否使用图编译、计算padding、DP跨卡同步token数 ==========
                (
                    cudagraph_mode,        # ACL图模式 NONE/PIECEWISE/FULL
                    batch_desc,            # 批次描述：padding后token、请求数
                    should_ubatch,         # 是否开启微批（vllm-Ascend暂不支持）
                    num_tokens_across_dp,  # DP各卡对齐后的padding token数组
                    cudagraph_stats,       # 图运行统计指标
                ) = self._determine_batch_execution_and_padding(
                    num_tokens=num_tokens_unpadded,  # 当前调度批次所有请求真实 token 加和
                    num_reqs=num_reqs,
                ...
                num_tokens_padded = batch_desc.num_tokens  # padding后总token（图对齐用）
                num_reqs_padded = batch_desc.num_reqs if batch_desc.num_reqs is not None else num_reqs  # padding请求数
                # 微批分片（Ascend未启用）
                ...
                pad_attn = cudagraph_mode == CUDAGraphMode.FULL  # FULL图需要padding注意力元数据

                # ========== 13.Mamba缓存预处理 ==========
                # 对齐模式Mamba状态拷贝前置处理，参考社区PR#30877后续会补充后置逻辑
                if self.cache_config.mamba_cache_mode == "align":
                    ...
                    # 同步已接受token计数到NPU，Mamba内核读取正确起始slot
                    self.num_accepted_tokens.np[:num_reqs] = (
                        self.input_batch.num_accepted_tokens_cpu[:num_reqs]
                    )
                ...
                use_spec_decode = len(scheduler_output.scheduled_spec_decode_tokens) > 0  # 是否投机解码
                ubatch_slices_attn = ubatch_slices_padded if pad_attn else ubatch_slices  # 注意力微批分片

                # ========== 15.FULL图/SP序列并行时填充query_start_loc适配FIA内核 ==========
                if (
                ...
                # ========== 16.构建完整注意力元数据（各层KV、slot、seq_len等） ==========
            ...
            # ========== 17.多模态输入embedding、encoder张量预处理 ==========
            (
                input_ids,
                inputs_embeds,
                positions,
                ...
                scheduler_output,
                num_tokens_padded  # self.input_ids.gpu[:num_tokens_padded]
                if not (self.use_cp and self.pcp_manager.pcp_use_hybrid_attn)
                else total_num_scheduled_tokens,
                intermediate_tensors,
            )

            # 更新全局RoPE cos/sin缓存（NPU算子全局复用）
        ...
        # ========== 19.计算KV缩放时强制关闭图编译 ==========
        # KV动态缩放算子不兼容ACL图，切换eager
        if self.calculate_kv_scales:  # type: ignore[has-type]
            cudagraph_mode = CUDAGraphMode.NONE
            self.calculate_kv_scales = False  # 仅首次计算，标记关闭
        ...
        num_encoder_reqs = len(scheduler_output.scheduled_encoder_inputs)
        has_encoder_input = self.model_config.is_encoder_decoder and num_encoder_reqs  # 是否包含编码器输入
        clear_kv_metadata = self.speculative_config is None  # 无投机解码则清理KV元数据

        # ========== 21.核心模型前向上下文管理器 ==========
        # 1. forward性能埋点
        # 2. 初始化昇腾专属前向上下文（attn元数据、图参数、PCP信息）
        # 3. KV迁移连接器上下文，延迟释放KV缓存（投机解码场景）
        with (
            ...
            # 【真正执行模型前向】调用封装好的_model_forward函数
        ...
        with record_function_or_nullcontext("post process"):
            aux_hidden_states = None  # 辅助隐藏态（Eagle3等投机模型）
            # 模型返回元组：主hidden_states + 辅助层输出
            if self.use_aux_hidden_state_outputs:
                hidden_states, aux_hidden_states = hidden_states

            # PCP并行：恢复拆分后的完整hidden_states张量
            if self.pcp_size > 1:
            ...
            if not self.broadcast_pp_output:
                ...
                # 池化模型（嵌入/分类任务）直接返回池化输出，不采样token
                ...
                # 取每个请求末尾token的hidden_states用于计算logits
            ...
            # ========== 25.缓存推理中间状态，交给sample_tokens采样 ==========
            # 把logits、隐藏态、注意力元数据等存入state，本函数返回None
        ...
        # 前向跑完后执行延迟状态修正（异步调度残留计数同步）
        if deferred_state_corrections_fn:
            deferred_state_corrections_fn()

        # execute_model仅做模型前向、缓存状态，不执行采样，返回None
        return None
    ...
    def _model_forward(
            ...
            intermediate_tensors: IntermediateTensors | None = None,
            inputs_embeds: torch.Tensor | None = None,** model_kwargs: dict[str, Any],
    ):
        # 断言：模型必须已加载，为空直接报错
        assert self.model is not None
        # 获取全局前向上下文，存放KV缓存、注意力元数据、图运行参数等推理上下文
        forward_context = get_forward_context()
        # 上下文不能为空，否则推理环境异常
        assert forward_context is not None

        # 组装传给模型forward的全部入参字典：基础张量 + 外部扩展参数
        model_inputs: dict[str, Any] = {
            "input_ids": input_ids,                # 输入token id张量
            "positions": positions,                # 位置编码position张量
            "intermediate_tensors": intermediate_tensors,  # PP流水线并行中间隐藏态
            "inputs_embeds": inputs_embeds,        # 多模态输入embedding（图文模型使用）
            **model_kwargs,                        # 解包额外动态参数（注意力元数据、kv缓存等）
        }
        # 用partial封装后，直接 run() 就会自动传入所有提前绑定的参数
        run_model = partial(self.model, **model_inputs)

        # 判断是否开启ENPU昇腾分段编译特性
        if self.enable_enpu:
            # ENPU场景需要先更新ACL图运行参数，再执行模型
            self._update_full_graph_params_if_needed(
                forward_context, num_tokens_padded, positions
            )
            # 真正执行模型前向传播，等价 self.model(所有绑定参数)
            hidden_states = run_model()
        else:
            # 普通图/ eager 模式：先跑模型，再更新图动态参数
            hidden_states = run_model()
            self._update_full_graph_params_if_needed(
                forward_context, num_tokens_padded, positions
            )

        # 判断是否开启flashcomm序列并行SP，且不是流水线中间张量
        if forward_context.flash_comm_v1_enabled and not isinstance(hidden_states, IntermediateTensors):
            # 执行TP全聚合，把各卡分片的hidden_states合并成完整张量
            hidden_states = self._all_gather_hidden_states_and_aux(hidden_states)
        # 返回模型输出的顶层隐藏态，后续用于计算logits采样token
        return hidden_states
